...
const VisaForm4 = () => {
  ...
  const validateForm = (): FormErrors => {
    ...
    if (!formData.purposeofJourney) newErrors.purposeofJourney = "Purpose of Entry required.";
    if (!formData.noofEntry) newErrors.noofEntry = "Mandatory.";
    if (!formData.expentryDate) newErrors.expentryDate = "Entry date required.";
    if (!formData.noofTravelDoc) newErrors.noofTravelDoc = "Travel document entered.";
    return newErrors;
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await axios.put(`${API_URL}/api/visa/${storedFormId}`, visaDetailsJson);
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="bg-white bg-opacity-90 rounded-xl shadow-lg overflow-hidden">
            <div className="p-6 md:p-8">
              ...
              <h4 className="text-lg mb-6 text-gray-600 border-b pb-2">Visa Information:</h4>

              <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {/* Visa Type */}
                <div className="flex flex-col">
                  <label className="text-gray-700 font-medium mb-1">Visa Type:</label>
                  <select
                    name="visaType"
                  ...
                  {errors.visaType && (
                    <span className="text-red-500 text-sm mt-1">{errors.visaType}</span>
                  )}
                </div>

                {/* Purpose of Journey */}
                <div className="flex flex-col">
                  <label className="text-gray-700 font-medium mb-1">Purpose of Journey:</label>
                  <input
                    type="text"
                    ...
                    onChange={handleChange}
                    className={`p-3 rounded-lg border focus:ring-2 focus:ring-green-500 focus:border-transparent ${
                      errors.purposeofJourney ? "border-red-500" : "border-gray-300"
                    }`}
                    placeholder="Enter purpose of journey"
                  />
                  {errors.purposeofJourney && (
                    <span className="text-red-500 text-sm mt-1">{errors.purposeofJourney}</span>
                  )}
                </div>

                {/* Number of Entry */}
                <div className="flex flex-col">
                  <label className="text-gray-700 font-medium mb-1">Number of Entry:</label>
                  <select
                    name="noofEntry"
                  ...
                  {errors.noofEntry && (
                    <span className="text-red-500 text-sm mt-1">{errors.noofEntry}</span>
                  )}
                </div>

                {/* Expected Entry Date */}
                <div className="flex flex-col">
                  <label className="text-gray-700 font-medium mb-1">Expected Entry Date:</label>
                  <input
                    type="date"
                    name="expentryDate"
                    ...
                    onChange={handleChange}
                    className={`p-3 rounded-lg border focus:ring-2 focus:ring-green-500 focus:border-transparent ${
                      errors.expentryDate ? "border-red-500" : "border-gray-300"
                    }`}
                  />
                  {errors.expentryDate && (
                    <span className="text-red-500 text-sm mt-1">{errors.expentryDate}</span>
                  )}
                </div>

                {/* Number of Travel Document */}
                <div className="flex flex-col">
                  <label className="text-gray-700 font-medium mb-1">Number of Travel Document:</label>
                  <select
                    name="noofTravelDoc"
                    value={formData.noofTravelDoc}
                    onChange={handleChange}
                    className={`p-3 rounded-lg border focus:ring-2 focus:ring-green-500 focus:border-transparent ${
                      errors.noofTravelDoc ? "border-red-500" : "border-gray-300"
                    }`}
                  >
                    <option value="">-- Number of documents presented --</option>
                  ...
                  {errors.noofTravelDoc && (
                    <span className="text-red-500 text-sm mt-1">{errors.noofTravelDoc}</span>
                  )}
                </div>
